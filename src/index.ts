...
async function main() {
  ...
  if (!apiKey) {
    ...
    console.log(
      chalk.yellow(
        "Please set it in your environment variables or a .env file."
      )
    );
  ...
  try {
    ...
    // If --push is passed and with --commit then push the current branch to origin
    ...
    if (shouldCommit) {
      ...
      console.log(
        chalk.white(`git commit -m "${message.replace(/"/g, '\\"')}"`)
      );

      console.log(
        chalk.dim("The commit message has also been copied to your clipboard.")
      );
    }
  } catch (error) {
